...
import gui.TemoignageApp;
import gui.addRando;
import gui.listrando;
...
public class HomeGuide {
    ...
    public HomeGuide(Resources theme) {
        ...
        f.getToolbar().addMaterialCommandToSideMenu("Evenement", FontImage.MATERIAL_EVENT, e -> {
                    themeG = UIManager.initFirstTheme("/themeG");

            listrando l = new listrando(themeG);
            l.getF().show();
        });
        f.getToolbar().addMaterialCommandToSideMenu("Temoignage", FontImage.MATERIAL_NOTE_ADD, e -> {
                    themeT = UIManager.initFirstTheme("/themeTem");

            TemoignageApp p = new TemoignageApp(themeT);
            p.getF().show();
        });
