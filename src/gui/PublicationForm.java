...
public class PublicationForm {
    ...
    Command menuAdd, menuDelete, navigationPub, navigationMem, navigationPhoto,retour;
    Resources themelog;
    ...
    public void displayPublication(Resources theme) {
        ...
        Image iback = theme.getImage("bac.png");
        ...
        retour = new Command("Retour vers mes groupes",iback);
        ...
        f.getToolbar().addCommandToSideMenu(retour);
        f.addCommandListener((ActionEvent ev) -> {
            if (ev.getCommand() == retour) {
                themelog = UIManager.initFirstTheme("/themelog");
                        Home home = new Home(themelog);
                        home.getF().show();
            }
        ...
        btnpub.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (!fileName.equals("") && "".equals(txtpub.getText())) {
                    PublicationGr p1 = new PublicationGr(m, g, "\"\"", fileName, 0);
                    ps.addpub(p1);
                    ...
                    PublicationGr p = new PublicationGr(m, g, txtpub.getText(), "\"\"", 0);
                    ps.addpub(p);
                    displayPublication(theme);

                } else if (!fileName.equals("") && !"".equals(txtpub.getText())) {
                    PublicationGr p = new PublicationGr(m, g, txtpub.getText(), fileName, 0);
                    ps.addpub(p);
                    ImageG img = new ImageG(fileName, g, m);
                    new ImageService().storeUploadImg(img);
                    displayPublication(theme);
        ...
        ArrayList<PublicationGr> ap = ps.getListPub();
        if (ap.isEmpty()) {
    ...
    public Container showOnlyPub(PublicationGr i, Resources theme, Form f) {
        ...
        btnupd.addActionListener(e -> {
            ...
            txtpubli.addActionListener(ev -> {
                PublicationGr pub = new PublicationGr(i.getId(), txtpubli.getText());
                ps.update(pub);
                displayPublication(theme);
                txtpubli.setUIID("Container");
                txtpubli.setEditable(false);
    ...
    public Container showPubWithImg(PublicationGr i, Resources theme, Form f) {
        Container ctn1 = new Container(new BoxLayout(BoxLayout.X_AXIS));
        Container ctn2 = new Container(new BoxLayout(BoxLayout.Y_AXIS));
        Container ctn4 = new Container(new BoxLayout(BoxLayout.Y_AXIS));
        ...
        ctn4.add(lbl3);
        ctn4.add(lbl1);
        Image imgpub = URLImage.createToStorage(encoded, "img"+i.getPhoto(),"http://localhost/piService/" + i.getPhoto());
    ...
    public Container showOnlyImg(PublicationGr i, Resources theme, Form f) {
        Container ctn1 = new Container(new BoxLayout(BoxLayout.X_AXIS));
        Container ctn2 = new Container(new BoxLayout(BoxLayout.Y_AXIS));
        Container ctn4 = new Container(new BoxLayout(BoxLayout.Y_AXIS));
